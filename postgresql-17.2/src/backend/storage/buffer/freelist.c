	...
	*from_ring = false;
	...
	if (StrategyControl->firstFreeBuffer >= 0)
	{
		while (true)
		{
			...
			// BEGIN NEW CODE
			// Keep this in case of pinning being weird - usage count will always be zero
			// but if they thought they needed to double check pinning, I will keep this
			// END NEW CODE
			/*
	...
	for (int i = 0; i < NBuffers; i++)
	{
		...
		if (BUF_STATE_GET_REFCOUNT(local_buf_state) == 0)
		{
			// found better choice to evict (fewer uses or equal uses and older)
			if (buf_to_evict == NULL ||
					buf->usage_count < buf_to_evict->usage_count ||
					(buf->usage_count == buf_to_evict->usage_count &&
					 buf->last_use_time < buf_to_evict->last_use_time))
			{
				buf_to_evict = buf;
			}
		}
